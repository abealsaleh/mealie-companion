...
2. **Shopping List** - OurGroceries-style shopping list with category grouping, autocomplete from history, and tap-to-check
...
- Docker Compose
- A Mealie API token (generate in Mealie: User > API Tokens)
...
6. Enter your Mealie API token on first use (stored in browser localStorage).
...
- Date picker (defaults to today) and meal type selector
...
- **Autocomplete**: searches "Previously Bought" list (~1000+ items) as you type, auto-assigns categories from history
- **Tap to check/uncheck**: items move between active and checked sections
- **Clear checked**: deletes checked items and ensures they exist in the Previously Bought list for future autocomplete
...
- `POST /api/households/shopping/items` - add item
